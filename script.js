...
(() => {
	...
	const htmlRefs = {
		...
		speedDisplayCurrent: document.querySelector(".speed-display .current"),
		speedDisplayFadingOut: document.querySelector(".speed-display .fading-out")
	}
	...
	let continueClickedLastOn = -1;

	const assignNewFlashcard = () => {
		currentCard = createRandomFlashcard();
		htmlRefs.flashcard.classList.add("hide-answer");
		htmlRefs.jp.innerHTML = "　";
		htmlRefs.jpFuri.innerHTML = "　";
		htmlRefs.translation.innerHTML = currentCard.translation;
		htmlRefs.image.style.visibility = currentCard.image === null ? "hidden" : "visible";
		htmlRefs.image.src = currentCard.image === null ? "" : currentCard.image;
	}

	const showAnswer = () => {
		htmlRefs.flashcard.classList.remove("hide-answer");
		htmlRefs.jp.innerHTML = currentCard.japanese;
		htmlRefs.jpFuri.innerHTML = currentCard.furigana;
	}
	...
	const onContinueClicked = () => {
		...
		const sinceLast = time - continueClickedLastOn;
		if (timerStart === -1) {
			timerStart = time;
			window.setInterval(updateCounter, 30_000);
		}
		if (sinceLast > 300 && sinceLast < 40_000) { // don't count spam or long delays
			continueClickCount++;
		}
		continueClickedLastOn = time;
	};

	const updateCounter = () => {
		if (continueClickCount < 3 || timerStart === -1) return;
		const elapsedMs = document.timeline.currentTime - timerStart;
		const elapsedMinutes = elapsedMs / 60_000;
		const answersTotal = continueClickCount / 2;
		const secondsPerCard = elapsedMs / 1000 / answersTotal;

		const message = Math.floor(answersTotal) + " cards in " + elapsedMinutes.toFixed(1) + " min ("
			+ secondsPerCard.toFixed() + "s per answer)";
		const cur = htmlRefs.speedDisplayCurrent;
		const last = htmlRefs.speedDisplayFadingOut;

		last.textContent = cur.textContent;
		...
		requestAnimationFrame(() => {
			...
			requestAnimationFrame(() => {
				cur.style.transition = "opacity 1.5s ease-in-out";
				last.style.transition = "opacity 1.5s ease-in-out";
				cur.style.opacity = "1";
